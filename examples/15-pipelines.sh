...
show_pipe "$FK -H '\$revenue+0 > 15000 { print \$region, \$product, \$revenue }' $TMPDIR/sales.csv | $FK '{ by[\$1] += \$3 } END { for (r in by) out[r]=sprintf(\"  %s \$%.0f\",rpad(r,6),by[r]); asort(out); print out }'"
...
show_pipe "$FK -H '{ print \$product, \$revenue, \$region }' $TMPDIR/sales.csv | awk '\$2+0 > 15000' | $FK '{ printf \"  %-8s \$%-6s (%s)  hex=\$%s\n\", \$1, \$2, \$3, hex(\$2+0) }'"
...
show_pipe "paste <($FK -H '{ print \$product }' $TMPDIR/sales.csv) <($FK -H '{ print \"\$\" \$revenue }' $TMPDIR/sales.csv) | $FK '{ printf \"  %-10s %s\n\", \$1, \$2 }'"

echo ""
echo "Diff two transformations of the same data:"
diff --color=never \
    <($FK -H '{ print $region, $product }' "$TMPDIR/sales.csv" | sort) \
    <($FK -H '$revenue+0 > 15000 { print $region, $product }' "$TMPDIR/sales.csv" | sort) \
    | $FK '{ print "  " $0 }' || true
...
show_pipe "$FK '{ ips[\$1]++ } END { print ips }' $TMPDIR/access.log | xargs -I{} printf '  resolve {} → {}.example.com\n'"
