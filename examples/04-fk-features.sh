...
printf "2\n3\n4\n5\n" | show $FK '{ printf "%d ** 3 = %d\n", $1, $1 ** 3 }'
echo ""

echo "   Square roots via ** 0.5:"
printf "16\n25\n144\n" | show $FK '{ printf "sqrt(%d) = %g\n", $1, $1 ** 0.5 }'
echo ""

# ── Hex literals ─────────────────────────────────────────────────
echo "2) Hex literals:"
echo "x" | show $FK '{ printf "0xFF = %d, 0x1F = %d, 0xCAFE = %d\n", 0xFF, 0x1F, 0xCAFE }'
echo ""

# ── Unicode escapes ──────────────────────────────────────────────
echo "3) Unicode escape sequences:"
echo "x" | show $FK 'BEGIN { printf "\\u2192 is: \u2192\n\\u2764 is: \u2764\n\\u00e9 is: \u00e9\n" }'
echo ""

# ── Negative field indexes ───────────────────────────────────────
echo "4) Negative field indexes — access from the end:"
echo "alpha beta gamma delta epsilon" | show $FK '{
...
echo "x" | show $FK '{ status = system("echo \"  Hello from system()\""); print "  Exit status:", status }'
echo ""

# ── /dev/stderr ──────────────────────────────────────────────────
echo "8) /dev/stderr — separate output streams:"
echo "data" | show $FK '{ print "stdout: " $0; print "stderr: " $0 > "/dev/stderr" }' 2>/tmp/fk_stderr_demo
...
echo "café résumé naïve" | show $FK '{
    for (i = 1; i <= NF; i++)
...
show $FK '{ print "  ", FILENAME, $0; nextfile }' "$TMPDIR/f1.txt" "$TMPDIR/f2.txt"
echo ""

# ── FILENAME and FNR ────────────────────────────────────────────
echo "11) FILENAME and FNR — per-file record tracking:"
show $FK '{ printf "  %-20s FNR=%-3d NR=%-3d %s\n", FILENAME, FNR, NR, $0 }' "$TMPDIR/f1.txt" "$TMPDIR/f2.txt"
echo ""

# ── do-while ────────────────────────────────────────────────────
echo "12) do-while — runs body at least once:"
echo "5" | show $FK '{ n=$1; do { printf "  %d", n; n-- } while (n>0); print "" }'
echo ""
...
echo "foo bar foo baz foo" | show $FK '{
    print "  original:", $0
...
echo "x" | show $FK 'BEGIN { print "  HOME =", ENVIRON["HOME"] }'
echo ""
