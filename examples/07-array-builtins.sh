...
printf 'banana\napple\ncherry\napple\n' | show $FK '{ u[$1]++ } END { print u }'
echo "   Sequential (prints values in order):"
printf 'cherry\napple\nbanana\n' | show $FK '{ a[NR]=$1 } END { asort(a); print a }'
echo ""
...
echo "x" | show $FK 'BEGIN { seq(a, 1, 10); print "  1..10:", join(a, ",") }'
echo "x" | show $FK 'BEGIN { seq(a, 5, -5); print "  5..-5:", join(a, ",") }'
echo ""
...
show $FK -F: '!/^#/{ u[$1]++ } END { print u }' /etc/passwd | head -5
